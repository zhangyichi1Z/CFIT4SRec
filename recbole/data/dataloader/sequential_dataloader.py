...
class SequentialDataLoader(AbstractDataLoader):
    ...
    def data_preprocess(self):
        ...
        if self.config['model'] == 'DuoRec' or self.config['model'][:5] == 'Model':
    ...
    def _get_processed_data(self, index):
        ...
        if self.config['SSL_AUG'] == 'CL4SRec' and self.phase == 'train':
            ...
            self.paper3_aug1(cur_data, index)
        elif self.config['SSL_AUG'] == 'MyRec' and self.phase == 'train':
            self.myrec_aug(cur_data)
        elif self.config['SSL_AUG'] == 'MyRec4' and self.phase == 'train':
            self.duorec_aug(cur_data, index)
        elif self.config['SSL_AUG'] == 'paper3' and self.phase == 'train':
            self.paper3_aug(cur_data, index)

        if self.config['noise'] == 'OPEN' and self.phase == 'test':
    ...
    def paper3_aug1(self, cur_data, index):
        L = self.config['same_length']
        cur_same_target = self.same_target_index[index]
        res = []
        res_lengths = []
        if self.config['train_r'] < 1:
            length_index = self.static_item_id_list.shape[0]
        for i, targets in enumerate(cur_same_target):
            # in case there is no same-target sequence
            # don't know why this happens since the filtering has been applied
            if self.config['train_r'] < 1:
                targets = targets[np.where(targets < length_index)]
            if len(targets) == 0:
                sem_pos_seqs = cur_data['item_id_list'][[i] * L]
                sem_pos_lengths = cur_data['item_length'][[i] * L]
            else:
                sem_pos_seqs = self.static_item_id_list[np.random.choice(targets, L)]
                sem_pos_lengths = self.static_item_length[np.random.choice(targets, L)]
            res.append(sem_pos_seqs)
            res_lengths.append(sem_pos_lengths)
        s = torch.stack(res, dim=1)
        s_l = torch.stack(res_lengths, dim=1)
        cur_data.update(Interaction({'sem_aug': s, 'sem_aug_lengths': s_l}))

    def paper3_aug(self, cur_data, index):
        cur_same_target = self.same_target_index[index]
        for i, targets in enumerate(cur_same_target):
            # in case there is no same-target sequence
            # don't know why this happens since the filtering has been applied
            if len(targets) == 0:
                # sem_pos_seqs = self.static_item_id_list[-1]
                # sem_pos_lengths = self.static_item_length[-1]
                sem_pos_seqs = cur_data['item_id_list'][[i]]
                sem_pos_lengths = cur_data['item_length'][[i]]
                cur_data.update(Interaction(
                    {'sem_aug_{}'.format(i): sem_pos_seqs, 'sem_aug_lengths_{}'.format(i): sem_pos_lengths}))

            else:
                # sample_pos.append(np.random.choice(targets))
                # sample_pos.append(targets)

                sem_pos_seqs = self.static_item_id_list[np.random.choice(targets, 10)]
                sem_pos_lengths = self.static_item_length[np.random.choice(targets, 10)]
                cur_data.update(Interaction(
                    {'sem_aug_{}'.format(i): sem_pos_seqs, 'sem_aug_lengths_{}'.format(i): sem_pos_lengths}))
